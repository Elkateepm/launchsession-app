...
export default function SessionSheet({ title, subtitle, onClose, children, footer, width = 600, busy = false, bodyStyle = {} }) {
  ...
  return createPortal(<div onClick={() => !busy && onClose()} style={{ position: 'fixed', inset: 0, zIndex: 10800, background: 'rgba(15,23,42,.42)', display: 'flex', justifyContent: 'flex-end' }}>
    <section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} onClick={e => e.stopPropagation()} style={{ width: mobile ? '100%' : width, maxWidth: '100%', height: '100dvh', minWidth: 0, boxSizing: 'border-box', background: 'var(--surface)', color: 'var(--text)', display: 'flex', flexDirection: 'column', boxShadow: '-12px 0 50px #0F172A25', outline: 'none' }}>
      <header style={{ padding: '16px 20px', paddingTop: 'max(16px, env(safe-area-inset-top))', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', gap: 12 }}><div style={{ minWidth: 0, flex: 1 }}><h2 id={titleId} style={{ fontSize: 20, margin: '4px 0', letterSpacing: '-.4px', overflowWrap: 'anywhere' }}>{title}</h2>{subtitle && <p style={{ margin: '6px 0 0', color: 'var(--text3)', fontSize: 13, lineHeight: 1.5 }}>{subtitle}</p>}</div><button disabled={busy} aria-label="Close panel" onClick={onClose} style={{ ...flowButton, width: 44, flexShrink: 0, padding: 0, fontSize: 22 }}>×</button></header>
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: mobile ? 16 : 24, ...bodyStyle }}>{children}</div>
      {footer && <footer style={{ borderTop: '1px solid var(--border)', padding: '14px 20px calc(14px + env(safe-area-inset-bottom, 0px))', flexShrink: 0 }}>{footer}</footer>}
