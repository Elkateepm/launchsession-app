...
export default function SessionWizard({ org, session, bubbleDefs, onCancel, onPublished, onNavigate, initialType, initialTemplate, initialPlan, editSession }) {
  ...
  return screen(<div style={{ ...(compact ? { position: 'fixed', inset: 0, zIndex: 10700, height: '100dvh' } : { height: '100%', minHeight: 600 }), overflow: 'hidden', display: 'flex', flexDirection: 'column', background: 'var(--surface2)', color: 'var(--text)' }}>
    <header style={{ padding: compact ? '12px 16px' : '20px 28px', paddingTop: compact ? 'max(12px, env(safe-area-inset-top))' : 20, borderBottom: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 12, justifyContent: 'space-between', marginBottom: 12 }}><h1 ref={headingRef} tabIndex={-1} style={{ fontSize: compact ? 19 : 23, margin: 0, outline: 'none' }}>{isEditing ? 'Edit' : 'New'} {terms.session}</h1><button onClick={closeWizard} disabled={saving} style={{ ...flowButton, border: 0, color: 'var(--text3)' }}>Close</button></div>
      {compact ? <><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 10, fontSize: 13 }}><strong>{STEP_DEFS[step - 1]?.label}</strong><span style={{ color: 'var(--text3)' }}>Step {step} of {totalSteps}</span></div><div role="progressbar" aria-label="Planning progress" aria-valuenow={step} aria-valuemin={1} aria-valuemax={totalSteps} style={{ height: 4, background: 'var(--surface3)', borderRadius: 8 }}><div style={{ width: `${step / totalSteps * 100}%`, height: '100%', borderRadius: 8, background: primary }} /></div></> : <div style={{ display: 'flex', gap: 8 }}>{STEPS.map((name, i) => <StepDot key={name} n={i + 1} label={name} active={step === i + 1} done={step > i + 1} onClick={() => step > i + 1 && setStep(i + 1)} compact={false} color={primary} />)}</div>}
    </header>
