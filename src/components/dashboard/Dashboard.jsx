...
function NavItem({ icon, label, active, onClick, primary, collapsed, locked, badge }) {
    ...
    <button
      ...
      style={{
        ...
        background: active
          ...
          : hovered && !locked ? 'rgba(255,255,255,0.07)' : 'transparent',
        color: locked ? 'rgba(255,255,255,0.2)'
          : active ? '#fff'
          : hovered ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.5)',
        fontSize: 13, fontWeight: active ? 700 : 500,
        marginBottom: 2, cursor: locked ? 'default' : 'pointer',
        textAlign: 'left', transition: 'all 0.15s ease',
        position: 'relative',
        boxShadow: active ? `inset 0 0 0 1px ${primary}40` : 'none',
      }}
    >
      {/* Active left bar */}
      {active && !collapsed && (
        <div style={{ position: 'absolute', left: 0, top: '20%', bottom: '20%', width: 3, borderRadius: 99, background: primary, boxShadow: `0 0 8px ${primary}` }} />
      )}
      <span style={{
        fontSize: 17, width: 22, textAlign: 'center', flexShrink: 0,
        filter: locked ? 'grayscale(1)' : 'none',
        opacity: locked ? 0.3 : 1,
        transform: hovered && !active && !locked ? 'scale(1.15)' : 'scale(1)',
        transition: 'transform 0.15s, filter 0.15s',
        display: 'inline-block',
      }}>{icon}</span>
      {!collapsed && (
        <>
          <span style={{ flex: 1, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', letterSpacing: active ? 0 : 0.1 }}>{label}</span>
          {locked && <span style={{ fontSize: 9, opacity: 0.4, flexShrink: 0 }}>🔒</span>}
          {!locked && badge && <span style={{ background: badge.color || '#EF4444', color: '#fff', borderRadius: 99, padding: '1px 6px', fontSize: 9, fontWeight: 800, flexShrink: 0 }}>{badge.text}</span>}
          {active && !locked && <div style={{ width: 5, height: 5, borderRadius: '50%', background: primary, boxShadow: `0 0 6px ${primary}`, flexShrink: 0 }} />}
        </>
      )}
    </button>
  )
...
function NavSection({ title, children, collapsed, color }) {
  return (
    <div style={{ marginBottom: 6 }}>
      {!collapsed && title && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px 4px', marginBottom: 2 }}>
          <div style={{ fontSize: 9, fontWeight: 900, color: color || 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: 1.5 }}>{title}</div>
          <div style={{ flex: 1, height: 1, background: color ? color + '30' : 'rgba(255,255,255,0.05)' }} />
        </div>
      )}
      {children}
    </div>
...
function NavItem({ icon, label, active, onClick, badge, primary, collapsed, locked }) {
  ...
  const bg = active
    ? `linear-gradient(135deg, ${primary}30, ${primary}15)`
    : hovered && !locked ? 'rgba(255,255,255,0.06)' : 'transparent'
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 12, border: 'none',
        background: bg,
        color: locked ? 'rgba(255,255,255,0.18)' : active ? '#fff' : hovered ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.52)',
        cursor: locked ? 'default' : 'pointer',
        textAlign: 'left', transition: 'all 0.15s ease',
        position: 'relative', fontFamily: 'inherit',
        marginBottom: 2,
        boxShadow: active ? `0 2px 12px ${primary}25` : 'none',
        outline: 'none',
      }}
    >
      {/* Active left accent bar */}
      {active && (
        <div style={{ position: 'absolute', left: 0, top: '20%', bottom: '20%', width: 3, borderRadius: 99, background: primary, boxShadow: `0 0 10px ${primary}` }} />
      )}

      {/* Icon */}
      <span style={{
        fontSize: collapsed ? 19 : 17,
        width: collapsed ? '100%' : 22,
        textAlign: 'center',
        flexShrink: 0,
        opacity: locked ? 0.3 : 1,
        transform: hovered && !active && !locked ? 'scale(1.15)' : 'scale(1)',
      ...
      {!collapsed && (
        <>
          <span style={{ flex: 1, fontSize: 13, fontWeight: active ? 700 : 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{label}</span>
          {locked && <span style={{ fontSize: 9, opacity: 0.35, flexShrink: 0 }}>🔒</span>}
          {!locked && badge && (
            <span style={{ background: badge.color || '#EF4444', color: '#fff', borderRadius: 99, padding: '2px 7px', fontSize: 10, fontWeight: 800, flexShrink: 0, boxShadow: `0 2px 6px ${badge.color || '#EF4444'}60` }}>
              {badge.text}
            </span>
          )}
          {active && !locked && (
            <div style={{ width: 7, height: 7, borderRadius: '50%', background: primary, boxShadow: `0 0 8px ${primary}`, flexShrink: 0, animation: 'pulse-dot 2s infinite' }} />
          )}
        </>
      )}
    </button>
