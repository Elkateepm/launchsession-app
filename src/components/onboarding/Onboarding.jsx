...
const ORG_TYPES = [
  ...
  { value: 'after_school', label: '📚 After-School Programme' },
...
export default function Onboarding({ session, org, onComplete }) {
  ...
  const progress = { display: 'flex', gap: 6, marginBottom: 36 }
  const dot = (active, done) => ({ flex: 1, height: 4, borderRadius: 99, background: done ? '#3B82F6' : active ? '#60A5FA' : 'rgba(255,255,255,0.1)', transition: 'all 0.3s' })

  const Progress = () => (
    <div style={progress}>
      {[1,2,3,4].map(i => <div key={i} style={dot(step === i, step > i)} />)}
    </div>
  )
  ...
  if (step === 0) return (
    <div style={page}><div style={card}>
      ...
      <div style={sub}>Let's take 60 seconds to set up your LaunchSession workspace. We'll ask a few quick questions to personalise your experience.</div>
  ...
  if (step === 2) return (
    <div style={page}><div style={card}>
      ...
      <div style={sub}>An approximate number is fine — you can update this any time.</div>
  ...
  if (step === 3) return (
    <div style={page}><div style={card}>
      ...
      <div style={sub}>Enter your city or town.</div>
      <input style={{ width: '100%', boxSizing: 'border-box', padding: '14px 16px', borderRadius: 12, border: '1.5px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.05)', color: '#fff', fontSize: 15, outline: 'none', marginBottom: 28 }} placeholder="e.g. London" value={city} onChange={e => setCity(e.target.value)} onKeyDown={e => e.key === 'Enter' && city.trim() && next()} autoFocus />
  ...
  if (step === 4) return (
    <div style={page}><div style={card}>
      ...
      <button style={btn(!focus || saving)} onClick={handleComplete} disabled={!focus || saving}>{saving ? 'Setting up...' : 'Launch My Workspace 🚀'}</button>
      <button style={btnGhost} onClick={back}>← Back</button>
    </div></div>
  )
