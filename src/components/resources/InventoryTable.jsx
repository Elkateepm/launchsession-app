  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--text)' }}>Inventory</div>
      ...
      <div style={{ overflowX: 'auto' }}>
            ...
            {active.map(r => {
              ...
              return (
                <tr key={r.id} style={{ borderBottom: '1px solid var(--border-soft)' }}>
                  ...
                  <td style={td}><button onClick={() => onOpen(r)} style={{ background: 'none', border: 'none', fontWeight: 700, color: 'var(--text)', cursor: 'pointer', padding: 0, textAlign: 'left' }}>{r.name}</button></td>
                  ...
                  <td style={td}>
                    {adjustingId === r.id ? (
                      <div style={{ display: 'flex', gap: 4 }}>
                        <input type="number" value={adjustQty} onChange={e => setAdjustQty(e.target.value)} style={{ width: 50, padding: 3, borderRadius: 6, border: '1px solid var(--border)', fontSize: 11 }} autoFocus />
                        <button onClick={() => handleAdjust(r)} style={{ fontSize: 10, color: 'var(--ok-text)', background: 'none', border: 'none', cursor: 'pointer' }}><Icon name="✓" /></button>
                        <button onClick={() => setAdjustingId(null)} style={{ fontSize: 10, color: 'var(--text-faint)', background: 'none', border: 'none', cursor: 'pointer' }}><Icon name="✕" /></button>
                      </div>
                    ) : (
                      <button onClick={() => { setAdjustingId(r.id); setAdjustQty(String(r.quantity_available)) }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700, color: 'var(--text)', padding: 0 }}>{r.quantity_available}</button>
                    )}
                  </td>
        ...
        {active.length === 0 && <div style={{ textAlign: 'center', padding: 30, color: 'var(--text-faint)', fontSize: 13 }}>No resources in inventory yet.</div>}
      </div>
      ...
      {showMovements && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowMovements(false)}>
          <div style={{ background: 'var(--surface)', borderRadius: 16, padding: 20, width: 480, maxHeight: '70vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 12 }}>Stock Movements</div>
            {movements.length === 0 ? <div style={{ color: 'var(--text-faint)', fontSize: 13 }}>No movements recorded yet.</div> : movements.map(m => {
              const r = resources.find(x => x.id === m.resource_id)
              return (
                <div key={m.id} style={{ padding: '8px 0', borderBottom: '1px solid var(--border-soft)', fontSize: 12.5 }}>
                  <strong>{r?.name}</strong> — {m.movement_type} ({m.quantity >= 0 ? '+' : ''}{m.quantity}) · {new Date(m.created_at).toLocaleString('en-GB')}
                </div>
              )
            })}
            <button onClick={() => setShowMovements(false)} style={{ marginTop: 14, width: '100%', padding: 10, borderRadius: 9, border: '1px solid var(--border)', background: 'var(--surface)', fontWeight: 700, cursor: 'pointer' }}>Close</button>
          </div>
        </div>
      )}
    </div>
...
const th = { padding: '8px 10px', fontSize: 11, fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase' }
const td = { padding: '10px', color: 'var(--text2)' }
const ghostBtn = { padding: '7px 12px', borderRadius: 8, border: '1.5px solid var(--border)', background: 'var(--surface)', fontSize: 12, fontWeight: 700, color: 'var(--text2)', cursor: 'pointer' }
const miniBtn = { padding: '4px 8px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', fontSize: 10.5, fontWeight: 600, color: 'var(--text2)', cursor: 'pointer' }
