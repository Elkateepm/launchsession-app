...
export default function BookingRequests({ org, bookings, resources, sessions, staff, authUserId, onChanged }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: 20 }}>
      <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--text)', marginBottom: 14 }}>Booking Requests <span style={{ fontWeight: 500, color: 'var(--text-faint)', fontSize: 12 }}>({requests.length} pending)</span></div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {requests.map(b => {
          ...
          const conflict = findConflict(bookings, b.resource_id, b.start_time, b.end_time, b.id)
          return (
            <div key={b.id} style={{ border: '1px solid var(--border)', borderRadius: 12, padding: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--text)' }}>{resource?.name}</div>
