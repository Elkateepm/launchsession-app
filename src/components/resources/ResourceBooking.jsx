      ...
      <div style={{ minWidth: 700 }}>
        ...
        {HOURS.map(hour => (
          <div key={hour} style={{ display: 'grid', gridTemplateColumns: '80px repeat(7,1fr)', gap: 1, minHeight: 44, alignItems: 'stretch' }}>
            <div style={{ fontSize: 11, color: 'var(--text-faint)', paddingTop: 4, textAlign: 'right', paddingRight: 8, fontWeight: 600 }}>{hour}:00</div>
            {days.map(day => {
    ...
    <div>
      ...
      {showAddResource && (
        <div style={{ background: 'var(--info-bg)', border: '1.5px solid var(--info-border)', borderRadius: 16, padding: 18, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 12 }}>
            ...
            <div><label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>CAPACITY</label><input type="number" value={newResource.capacity} onChange={e => setNewResource(n => ({ ...n, capacity: e.target.value }))} placeholder="e.g. 30" style={inp} /></div>
      ...
      {showBook && (
        <div onClick={() => setShowBook(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: 20, width: '100%', maxWidth: 460, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div style={{ fontSize: 17, fontWeight: 800 }}><Icon name="📅" /> Make a Booking</div>
              <button onClick={() => setShowBook(false)} style={{ background: 'none', border: 'none', fontSize: 22, color: 'var(--text-faint)', cursor: 'pointer' }}>×</button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div><label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>RESOURCE</label>
              ...
              <div><label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>DATE</label><input type="date" value={bookingForm.date} onChange={e => setBookingForm(f => ({ ...f, date: e.target.value }))} style={inp} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 12 }}>
                <div><label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>START</label>
                ...
                <div><label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', display: 'block', marginBottom: 4 }}>END</label>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        <button onClick={() => setCurrentWeek(d => addDays(d, -7))} style={{ padding: '7px 14px', borderRadius: 10, border: '1.5px solid var(--border)', background: 'var(--surface)', fontWeight: 700, cursor: 'pointer' }}><Icon name="←" /> Prev</button>
      ...
      {resources.length === 0 ? (
        <div style={{ padding: 60, textAlign: 'center', background: 'var(--surface2)', borderRadius: 16, border: '1.5px dashed var(--border)' }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--text-faint)', marginBottom: 20 }}>Add rooms, vehicles, equipment and more to start booking</div>
        ...
        <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-faint)' }}>Loading schedule...</div>
      ) : (
