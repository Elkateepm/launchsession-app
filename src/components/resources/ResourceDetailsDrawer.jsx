  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', justifyContent: 'flex-end' }} onClick={onClose}>
      <div style={{ width: 460, maxWidth: '100%', height: '100%', background: 'var(--surface)', overflowY: 'auto', padding: 24 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', marginBottom: 4 }}>{resource.name}</div>
        <div style={{ fontSize: 12.5, color: 'var(--text3)', marginBottom: 10 }}>{cat.icon} {cat.label}{resource.location ? ` · 📍 ${resource.location}` : ''}</div>
        ...
        {resource.description && <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6, marginBottom: 16 }}>{resource.description}</div>}
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 22, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => onEdit(resource)} style={{ flex: 1, minWidth: 100, padding: '10px 14px', borderRadius: 9, border: '1.5px solid var(--border)', background: 'var(--surface)', color: 'var(--text2)', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>Edit</button>
          <button onClick={handleMarkUnavailable} style={{ flex: 1, minWidth: 130, padding: '10px 14px', borderRadius: 9, border: '1.5px solid var(--border)', background: 'var(--surface)', color: 'var(--text2)', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>
            {resource.status === 'unavailable' ? 'Mark Available' : 'Mark Unavailable'}
          </button>
        </div>

        <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text)', marginBottom: 8 }}>Upcoming bookings ({upcoming.length})</div>
        {loading ? <div style={{ fontSize: 12, color: 'var(--text-faint)' }}>Loading...</div> : upcoming.length === 0 ? (
          <div style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 16 }}>Nothing scheduled.</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            {upcoming.map(b => (
              <div key={b.id} style={{ background: 'var(--surface2)', borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
        ...
        {checkoutHistory.length > 0 && (
          <>
            <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text)', marginBottom: 8 }}>Check-out history</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
              {checkoutHistory.slice(0, 5).map(c => (
                <div key={c.id} style={{ background: 'var(--surface2)', borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
        ...
        {past.length > 0 && (
          <>
            <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text)', marginBottom: 8 }}>Past bookings</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 20 }}>
              {past.slice(0, 5).map(b => (
                <div key={b.id} style={{ fontSize: 12, color: 'var(--text3)' }}>{fmtDate(b.start_time)} · {fmtTime(b.start_time)}–{fmtTime(b.end_time)}</div>
              ))}
            </div>
          </>
        )}
...
function Stat({ label, value }) {
  return (
    <div style={{ background: 'var(--surface2)', borderRadius: 8, padding: '8px 10px' }}>
      <div style={{ fontSize: 10, color: 'var(--text-faint)', fontWeight: 700, textTransform: 'uppercase' }}>{label}</div>
