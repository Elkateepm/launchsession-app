...
export default function MemberAccess({ member, org, viewerRole, showToast }) {
  ...
  const setLevel = async (moduleKey, value) => {
    ...
    let error
    if (value === 'inherit') {
      ({ error } = await supabase.from('module_access_grants')
        .delete().eq('user_id', member.id).eq('module_key', moduleKey))
      if (!error) setGrants(g => { const n = { ...g }; delete n[moduleKey]; return n })
    } else {
      ({ error } = await supabase.from('module_access_grants')
      ...
      if (!error) setGrants(g => ({ ...g, [moduleKey]: value }))
    }
    setSaving(null)
    // The trigger raises a readable message for every refusal, so surface it
