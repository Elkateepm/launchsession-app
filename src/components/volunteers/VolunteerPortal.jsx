...
const typeConfig = {
  activity: { color:'#1B9AAA', icon:'ti-run' },
  trip:     { color:'#417505', icon:'ti-bus' },
  workshop: { color:'#F0A500', icon:'ti-tools' },
  holiday:  { color:'#9B59B6', icon:'ti-beach' },
}
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg, #0D1B2A 0%, #1B2A4A 50%, #9B59B6 100%)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:'#fff', borderRadius:24, width:'100%', maxWidth:420, padding:28, boxShadow:'0 25px 60px rgba(0,0,0,0.3)' }}>
        ...
        <button onClick={handleSave} disabled={!form.full_name.trim() || saving} style={{ width:'100%', padding:14, borderRadius:14, border:'none', background:form.full_name.trim()?primary:'var(--muted)', color:'#fff', fontSize:16, fontWeight:900, cursor:form.full_name.trim()?'pointer':'default', fontFamily:'var(--font-display)' }}>
    ...
    <div>
      ...
      <div style={{ background:'#0D1B2A', borderRadius:22, overflow:'hidden', marginBottom:14 }}>
        <div style={{ background:'linear-gradient(135deg, #1B2A4A 0%, #2D1B69 60%, #9B59B6 100%)', padding:'22px 18px 18px', position:'relative' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:16, position:'relative', zIndex:1 }}>
            <div style={{ position:'relative', flexShrink:0 }}>
              <div style={{ width:72, height:72, borderRadius:20, background:'#9B59B6', border:'3px solid rgba(245,208,0,0.4)', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
                {photoUrl?<img src={photoUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />:<span style={{ fontSize:24, fontWeight:900, color:'#fff', fontFamily:'var(--font-display)' }}>{(form.full_name||'?').split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2)}</span>}
              </div>
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontSize:22, fontWeight:900, fontFamily:'var(--font-display)', color:'#fff', marginBottom:3 }}>{form.full_name||'Volunteer'}</div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:14 }}>
            {[{val:hours.toFixed(1),label:'Hours',color:'#F5D000'},{val:sessions,label:'Sessions',color:'#9FE1CB'},{val:streak,label:'Streak',color:'#CECBF6'}].map(s=>(
          ...
          <div style={{ background:'rgba(255,255,255,0.07)', borderRadius:10, padding:'10px 12px' }}>
            ...
            <div style={{ background:'rgba(255,255,255,0.12)', borderRadius:99, height:6 }}>
              <div style={{ background:'linear-gradient(90deg, #F5D000, #1B9AAA)', width:progress+'%', height:'100%', borderRadius:99 }} />
            </div>
          </div>
        </div>
      </div>
      {form.availability?.length>0&&<div style={{ background:'#fff', borderRadius:18, border:'1.5px solid var(--border)', padding:'16px', marginBottom:12 }}><div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}><div style={{ width:30, height:30, borderRadius:9, background:'#E8F7F9', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-calendar-heart" style={{ fontSize:15, color:'#1B9AAA' }} /></div><span style={{ fontSize:13, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)' }}>When I can help</span></div><div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>{form.availability.map(d=><div key={d} style={{ display:'flex', alignItems:'center', gap:5, background:'#E8F7F9', border:'1.5px solid #1B9AAA', borderRadius:12, padding:'7px 14px' }}><i className="ti ti-sun" style={{ fontSize:13, color:'#1B9AAA' }} /><span style={{ fontSize:12, fontWeight:700, color:'#085041' }}>{d.slice(0,3)}</span></div>)}</div></div>}
      {sports.length>0&&<div style={{ background:'#fff', borderRadius:18, border:'1.5px solid var(--border)', padding:'16px', marginBottom:12 }}><div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}><div style={{ width:30, height:30, borderRadius:9, background:'#F5F0FF', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-bolt" style={{ fontSize:15, color:'#9B59B6' }} /></div><span style={{ fontSize:13, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)' }}>My sporting superpowers</span></div><div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>{sports.map(s=><span key={s} style={{ display:'flex', alignItems:'center', gap:5, background:'#F5F0FF', color:'#534AB7', borderRadius:99, padding:'6px 14px', fontSize:12, fontWeight:800, border:'1.5px solid #AFA9EC' }}>{SPORT_ICONS[s]&&<i className={'ti '+SPORT_ICONS[s]} style={{ fontSize:13 }} />}{s}</span>)}</div></div>}
      {languages.length>0&&<div style={{ background:'#fff', borderRadius:18, border:'1.5px solid var(--border)', padding:'16px', marginBottom:12 }}><div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}><div style={{ width:30, height:30, borderRadius:9, background:'#E1F5EE', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-world" style={{ fontSize:15, color:'#0F6E56' }} /></div><span style={{ fontSize:13, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)' }}>Languages I speak</span></div><div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>{languages.map(l=><span key={l} style={{ display:'flex', alignItems:'center', gap:5, background:'#E1F5EE', color:'#085041', borderRadius:99, padding:'6px 14px', fontSize:12, fontWeight:800, border:'1.5px solid #5DCAA5' }}><i className="ti ti-language" style={{ fontSize:13 }} />{l}</span>)}</div></div>}
      {customSkills.length>0&&<div style={{ background:'#fff', borderRadius:18, border:'1.5px solid var(--border)', padding:'16px', marginBottom:12 }}><div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}><div style={{ width:30, height:30, borderRadius:9, background:'#FFF8E6', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-sparkles" style={{ fontSize:15, color:'#F0A500' }} /></div><span style={{ fontSize:13, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)' }}>Other skills</span></div><div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>{customSkills.map(s=><span key={s} style={{ background:'#FFF8E6', color:'#B8860B', borderRadius:99, padding:'6px 14px', fontSize:12, fontWeight:800, border:'1.5px solid #FAC775' }}>{s}</span>)}</div></div>}
      <div style={{ background:'#fff', borderRadius:18, border:'1.5px solid '+(form.emergency_contact_name?'#9FE1CB':'#FFB3B3'), padding:'16px', marginBottom:14 }}>
        <button onClick={() => setShowPrivate(!showPrivate)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', background:'none', border:'none', cursor:'pointer', padding:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}><div style={{ width:30, height:30, borderRadius:9, background:form.emergency_contact_name?'#EDFAED':'#FFF0F0', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-shield-heart" style={{ fontSize:15, color:form.emergency_contact_name?'#417505':'#C00' }} /></div><div><span style={{ fontSize:13, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)' }}>Safeguarding contact</span>{!form.emergency_contact_name&&<div style={{ fontSize:10, color:'#C00', fontWeight:600 }}>Please add a trusted contact</div>}</div></div>
          <i className={'ti ti-chevron-'+(showPrivate?'up':'down')} style={{ fontSize:15, color:'var(--muted)' }} />
        </button>
        {showPrivate&&<div style={{ marginTop:12, paddingTop:12, borderTop:'1px solid var(--border)' }}>{form.emergency_contact_name?<div style={{ background:'#EDFAED', borderRadius:12, padding:'12px 14px', border:'1.5px solid #9FE1CB' }}><div style={{ fontSize:10, fontWeight:800, color:'#417505', textTransform:'uppercase', letterSpacing:0.5, marginBottom:6 }}>Trusted contact</div><div style={{ fontSize:14, fontWeight:800, color:'var(--black)', marginBottom:2 }}>{form.emergency_contact_name}</div><div style={{ fontSize:12, color:'var(--muted)', fontWeight:600 }}>{form.emergency_contact_phone}</div></div>:<div style={{ fontSize:12, color:'#C00', fontWeight:600, padding:'8px 0' }}>No safeguarding contact added yet. Edit your profile to add one.</div>}</div>}
      </div>
      <div style={{ background:'#0D1B2A', borderRadius:18, padding:'16px', marginBottom:14 }}>
        <div style={{ fontSize:10, fontWeight:800, color:'#F5D000', textTransform:'uppercase', letterSpacing:1.5, marginBottom:12, display:'flex', alignItems:'center', gap:6 }}><i className="ti ti-heart" style={{ fontSize:12 }} />Memory Moments</div>
      ...
      <button onClick={() => setEditMode(true)} style={{ width:'100%', padding:14, borderRadius:14, border:'none', background:'linear-gradient(135deg, #9B59B6, #7B2D8B)', color:'#fff', fontSize:15, fontWeight:900, cursor:'pointer', fontFamily:'var(--font-display)', marginBottom:20, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}><i className="ti ti-pencil" style={{ fontSize:16 }} />Edit my profile</button>
    </div>
  )

  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}><div style={{ fontSize:16, fontWeight:900, fontFamily:'var(--font-display)', color:'var(--black)' }}>Edit Profile</div><button onClick={() => setEditMode(false)} style={{ background:'var(--grey-bg)', border:'none', borderRadius:10, padding:'7px 14px', fontSize:12, fontWeight:700, color:'var(--muted)', cursor:'pointer' }}>Cancel</button></div>
      <SectionCard>
      ...
      <SectionCard borderColor={form.emergency_contact_name&&form.emergency_contact_phone?'#9FE1CB':'#FFB3B3'}>
        ...
        <div style={{ background:form.emergency_contact_name&&form.emergency_contact_phone?'#E1F5EE':'#FFF0F0', borderRadius:10, padding:'9px 12px', marginBottom:12 }}><div style={{ fontSize:12, color:form.emergency_contact_name&&form.emergency_contact_phone?'#085041':'#C00', fontWeight:600 }}>{form.emergency_contact_name&&form.emergency_contact_phone?'Great — we have a trusted adult we can reach if needed':'Just so we always have a trusted adult we can contact if needed'}</div></div>
      ...
      <SectionCard>
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
          {DAYS.map(d => { const on=form.availability.includes(d); return <button key={d} onClick={() => toggleDay(d)} style={{ padding:'8px 14px', borderRadius:99, border:on?'2px solid #1B9AAA':'1.5px solid var(--border)', background:on?'#E8F7F9':'#fff', fontSize:12, fontWeight:700, color:on?'#1B9AAA':'var(--muted)', cursor:'pointer' }}>{d.slice(0,3)}</button> })}
        </div>
      </SectionCard>
      <SectionCard>
        <SectionHead icon="ti-users-group" title="Age groups I love" filled={(form.age_groups||[]).length>0} />
        <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
          {['4-6','7-9','10-12','All ages'].map(a => { const on=form.age_groups?.includes(a); return <button key={a} onClick={() => toggleAge(a)} style={{ padding:'8px 16px', borderRadius:99, border:on?'2px solid #F0A500':'1.5px solid var(--border)', background:on?'#FFF8E6':'#fff', fontSize:12, fontWeight:700, color:on?'#B8860B':'var(--muted)', cursor:'pointer' }}>{a}</button> })}
        </div>
      </SectionCard>
      <SectionCard>
        <SectionHead icon="ti-trophy" title="Sporting superpowers" filled={(form.skills||[]).length>0} />
        <div style={{ fontSize:10, fontWeight:800, color:'var(--muted)', textTransform:'uppercase', letterSpacing:0.5, marginBottom:8 }}>Sports</div>
        <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:14 }}>{SPORTS.map(s => { const on=form.skills.includes(s); const ico=SPORT_ICONS[s]; return <button key={s} onClick={() => toggleSkill(s)} style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 12px', borderRadius:99, border:on?'2px solid #9B59B6':'1.5px solid var(--border)', background:on?'#F5F0FF':'#fff', fontSize:12, fontWeight:700, color:on?'#9B59B6':'var(--muted)', cursor:'pointer' }}>{ico&&<i className={'ti '+ico} style={{ fontSize:13 }} />}{s}</button> })}</div>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:14 }}>{LANGUAGES.map(l => { const on=form.skills.includes(l); return <button key={l} onClick={() => toggleSkill(l)} style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 12px', borderRadius:99, border:on?'2px solid #1B9AAA':'1.5px solid var(--border)', background:on?'#E8F7F9':'#fff', fontSize:12, fontWeight:700, color:on?'#1B9AAA':'var(--muted)', cursor:'pointer' }}>{on&&<i className="ti ti-world" style={{ fontSize:12 }} />}{l}</button> })}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:10, marginBottom:20 }}>
        ...
        <button onClick={handleSave} disabled={saving||!form.full_name.trim()} style={{ padding:14, borderRadius:14, border:'none', background:!form.full_name.trim()?'var(--muted)':'#9B59B6', color:'#fff', fontSize:15, fontWeight:900, cursor:form.full_name.trim()?'pointer':'default', fontFamily:'var(--font-display)' }}>{saving?'Saving...':'Save my profile'}</button>
      </div>
    </div>
  )
}
...
function BookTab({ futureSessions, sessionSignups, volunteerCounts, saving, handleBook, today }) {
  ...
  return (
    <div>
      {bookSuccess&&<div style={{ background:'linear-gradient(135deg, #417505, #1B9AAA)', borderRadius:16, padding:'16px 18px', marginBottom:14, display:'flex', alignItems:'center', gap:12 }}><div style={{ width:44, height:44, borderRadius:12, background:'rgba(255,255,255,0.2)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><i className="ti ti-confetti" style={{ fontSize:22, color:'#fff' }} /></div><div><div style={{ fontSize:15, fontWeight:900, color:'#fff', fontFamily:'var(--font-display)' }}>Session booked!</div><div style={{ fontSize:11, color:'rgba(255,255,255,0.8)', fontWeight:600 }}>Thank you for helping create happy memories</div></div></div>}
      <div style={{ marginBottom:16 }}><div style={{ fontSize:20, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)', marginBottom:4 }}>Find Your Next Session</div><div style={{ fontSize:12, color:'var(--muted)', fontWeight:600 }}>Join a session and help create happy memories</div></div>
      ...
      {confirmedBookings.length>0&&<div style={{ marginBottom:20 }}><div style={{ fontSize:12, fontWeight:900, color:'#417505', textTransform:'uppercase', letterSpacing:0.8, fontFamily:'var(--font-display)', marginBottom:10, display:'flex', alignItems:'center', gap:6 }}><i className="ti ti-circle-check" style={{ fontSize:14 }} />My Confirmed Sessions</div>{confirmedBookings.map(s=>{const cfg=typeConfig[s.session_type]||typeConfig.activity;const days=Math.ceil((new Date(s.date+'T00:00:00')-new Date())/(1000*60*60*24));return(<div key={s.id} style={{ background:'linear-gradient(135deg,#EDFAED,#E8F7F9)', borderRadius:14, border:'2px solid #417505', padding:'13px 14px', marginBottom:8, display:'flex', alignItems:'center', gap:12 }}><div style={{ width:42, height:42, borderRadius:12, background:cfg.color, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><i className={'ti '+cfg.icon} style={{ fontSize:18, color:'#fff' }} /></div><div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:800, fontFamily:'var(--font-display)', color:'var(--black)', marginBottom:2 }}>{s.title}</div><div style={{ fontSize:11, color:'var(--muted)', fontWeight:600 }}>{fmtDate(s.date)} · {s.start_time}</div></div><div style={{ textAlign:'center', flexShrink:0 }}><div style={{ fontSize:18, fontWeight:900, color:'#417505', lineHeight:1 }}>{days<=0?'Today!':days}</div>{days>0&&<div style={{ fontSize:9, color:'#417505', fontWeight:700, textTransform:'uppercase' }}>days</div>}</div></div>)})}</div>}
      {pendingBookings.length>0&&<div style={{ marginBottom:20 }}><div style={{ fontSize:12, fontWeight:900, color:'#B8860B', textTransform:'uppercase', letterSpacing:0.8, fontFamily:'var(--font-display)', marginBottom:10, display:'flex', alignItems:'center', gap:6 }}><i className="ti ti-clock" style={{ fontSize:14 }} />Awaiting Confirmation</div>{pendingBookings.map(s=>{const cfg=typeConfig[s.session_type]||typeConfig.activity;return(<div key={s.id} style={{ background:'#FFFBE6', borderRadius:14, border:'1.5px solid #F5D000', padding:'13px 14px', marginBottom:8, display:'flex', alignItems:'center', gap:12 }}><div style={{ width:42, height:42, borderRadius:12, background:cfg.color, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><i className={'ti '+cfg.icon} style={{ fontSize:18, color:'#fff' }} /></div><div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:800, fontFamily:'var(--font-display)', color:'var(--black)', marginBottom:2 }}>{s.title}</div><div style={{ fontSize:11, color:'var(--muted)', fontWeight:600 }}>{fmtDate(s.date)} · {s.start_time}</div></div><span style={{ background:'#FFF8E6', color:'#B8860B', borderRadius:99, padding:'4px 10px', fontSize:11, fontWeight:800, border:'1.5px solid #F5D000' }}>Pending</span></div>)})}</div>}
      <div style={{ fontSize:12, fontWeight:900, color:'var(--muted)', textTransform:'uppercase', letterSpacing:0.8, fontFamily:'var(--font-display)', marginBottom:12, display:'flex', alignItems:'center', gap:6 }}><i className="ti ti-compass" style={{ fontSize:14 }} />Available ({available.length})</div>
      {available.length===0?<div style={{ background:'#0D1B2A', borderRadius:18, padding:'28px 20px', textAlign:'center' }}><div style={{ width:56, height:56, borderRadius:16, background:'rgba(245,208,0,0.15)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px' }}><i className="ti ti-compass" style={{ fontSize:26, color:'#F5D000' }} /></div><div style={{ fontSize:15, fontWeight:900, color:'#fff', fontFamily:'var(--font-display)', marginBottom:6 }}>No sessions here!</div><div style={{ fontSize:12, color:'rgba(255,255,255,0.6)', fontWeight:600 }}>Try a different filter.</div></div>:available.map((s,idx)=>{const cfg=typeConfig[s.session_type]||typeConfig.activity;const isSaving=saving===s.id;const dateLabel=s.date===today?'Today':fmtDate(s.date);const count=volunteerCounts[s.id]||0;const limit=s.volunteer_limit;const isFull=limit&&count>=limit;const spotsLeft=limit?limit-count:null;const isFeatured=idx===0;return(<div key={s.id} style={{ background:isFeatured?'#0D1B2A':'#fff', borderRadius:18, border:isFeatured?'none':'1.5px solid var(--border)', padding:'16px', marginBottom:12, overflow:'hidden', position:'relative' }}>{isFeatured&&<div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg,#F5D000,#1B9AAA)' }} />}<div style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:12 }}><div style={{ width:52, height:52, borderRadius:14, background:cfg.color, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><i className={'ti '+cfg.icon} style={{ fontSize:24, color:'#fff' }} /></div><div style={{ flex:1, minWidth:0 }}>{isFeatured&&<div style={{ fontSize:10, fontWeight:800, color:'#F5D000', textTransform:'uppercase', letterSpacing:1, marginBottom:4 }}>Featured Session</div>}<div style={{ fontSize:16, fontWeight:900, fontFamily:'var(--font-display)', color:isFeatured?'#fff':'var(--black)', marginBottom:6 }}>{s.title}</div><div style={{ display:'flex', flexDirection:'column', gap:3 }}><div style={{ fontSize:12, fontWeight:600, color:isFeatured?'rgba(255,255,255,0.7)':'var(--muted)', display:'flex', alignItems:'center', gap:5 }}><i className="ti ti-calendar" style={{ fontSize:12, color:isFeatured?'#F5D000':'#9B59B6' }} />{dateLabel}</div>{s.start_time&&<div style={{ fontSize:12, fontWeight:600, color:isFeatured?'rgba(255,255,255,0.7)':'var(--muted)', display:'flex', alignItems:'center', gap:5 }}><i className="ti ti-clock" style={{ fontSize:12, color:isFeatured?'#F5D000':'#1B9AAA' }} />{s.start_time}{s.end_time?` – ${s.end_time}`:''}</div>}{s.location&&<div style={{ fontSize:12, fontWeight:600, color:isFeatured?'rgba(255,255,255,0.7)':'var(--muted)', display:'flex', alignItems:'center', gap:5 }}><i className="ti ti-map-pin" style={{ fontSize:12, color:isFeatured?'#F5D000':'#F0A500' }} />{s.location.split(',')[0]}</div>}</div></div></div>{spotsLeft!==null&&<div style={{ display:'flex', gap:6, marginBottom:12 }}><span style={{ background:isFull?'#FFF0F0':'#EDFAED', color:isFull?'#C00':'#417505', borderRadius:99, padding:'4px 10px', fontSize:11, fontWeight:800 }}>{isFull?'Full':`${spotsLeft} spot${spotsLeft!==1?'s':''} left`}</span></div>}<button onClick={() => !isFull&&setBookConfirm(s)} disabled={isSaving||isFull} style={{ width:'100%', padding:'12px', borderRadius:12, border:'none', background:isFull?(isFeatured?'rgba(255,255,255,0.1)':'#f0f0f0'):isFeatured?'#F5D000':'#9B59B6', color:isFull?(isFeatured?'rgba(255,255,255,0.4)':'var(--muted)'):isFeatured?'#0D1B2A':'#fff', fontSize:14, fontWeight:900, cursor:isFull?'default':'pointer', fontFamily:'var(--font-display)', display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>{isSaving?'...':(isFull?'Session Full':<><i className="ti ti-heart-plus" style={{ fontSize:15 }} />{isFeatured?'Join This Session':'Join Session'}</>)}</button></div>)})}
      {bookConfirm&&<div onClick={() => setBookConfirm(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:200, display:'flex', alignItems:'flex-end' }}><div onClick={e => e.stopPropagation()} style={{ background:'#fff', borderRadius:'20px 20px 0 0', width:'100%', padding:'20px 20px 32px' }}><div style={{ width:36, height:4, background:'var(--border)', borderRadius:2, margin:'0 auto 16px' }} /><div style={{ fontSize:16, fontWeight:900, fontFamily:'var(--font-display)', marginBottom:8 }}>Book this session?</div><div style={{ fontSize:14, color:'var(--muted)', fontWeight:600, marginBottom:20 }}>{bookConfirm.title} · {fmtDate(bookConfirm.date)}</div><button onClick={() => doBook(bookConfirm)} style={{ width:'100%', padding:'14px', borderRadius:14, border:'none', background:'linear-gradient(135deg,#9B59B6,#1B9AAA)', color:'#fff', fontSize:15, fontWeight:900, cursor:'pointer', fontFamily:'var(--font-display)', display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:10 }}><i className="ti ti-heart-plus" style={{ fontSize:16 }} />Confirm Booking</button><button onClick={() => setBookConfirm(null)} style={{ width:'100%', padding:'12px', borderRadius:14, border:'1.5px solid var(--border)', background:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', color:'var(--muted)' }}>Cancel</button></div></div>}
    </div>
  )
}
  ...
  return (
    <div>
      <div style={{ background:'linear-gradient(135deg, #0D1B2A 0%, #1B2A4A 100%)', borderRadius:20, padding:'20px', marginBottom:20, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ marginBottom:14 }}>
          <div style={{ display:'flex', alignItems:'baseline', gap:8, marginBottom:4 }}><div style={{ fontSize:48, fontWeight:900, color:'#F5D000', fontFamily:'var(--font-display)', lineHeight:1 }}>{youngPeople}</div><div style={{ fontSize:14, fontWeight:700, color:'rgba(255,255,255,0.6)' }}>young people</div></div>
          <div style={{ fontSize:13, color:'rgba(255,255,255,0.75)', fontWeight:500, lineHeight:1.5 }}>Thanks to your support, you've helped create happy memories for young people. ❤️</div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:14 }}>
          <div style={{ background:'rgba(255,255,255,0.07)', borderRadius:12, padding:'12px' }}><div style={{ fontSize:22, fontWeight:900, color:'#1B9AAA', fontFamily:'var(--font-display)', lineHeight:1, marginBottom:4 }}>{totalHours.toFixed(1)}</div><div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.5)', textTransform:'uppercase', letterSpacing:0.5 }}>⏱️ Hours Given</div></div>
          <div style={{ background:'rgba(255,255,255,0.07)', borderRadius:12, padding:'12px' }}><div style={{ fontSize:22, fontWeight:900, color:'#C9860F', fontFamily:'var(--font-display)', lineHeight:1, marginBottom:4 }}>{completed.length}</div><div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.5)', textTransform:'uppercase', letterSpacing:0.5 }}>✅ Sessions Done</div></div>
        </div>
      ...
      <div style={{ fontSize:14, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)', marginBottom:14, textTransform:'uppercase', letterSpacing:0.8 }}>Session Log</div>
      {attendance.length===0?<div style={{ background:'#fff', borderRadius:16, border:'1.5px solid var(--border)', padding:'32px', textAlign:'center', color:'var(--muted)' }}><i className="ti ti-clock-off" style={{ fontSize:36, display:'block', marginBottom:8 }} /><div style={{ fontSize:14, fontWeight:700 }}>No hours logged yet</div><div style={{ fontSize:12, marginTop:4 }}>Book sessions to start logging hours</div></div>:attendance.map(a=>{const sIn=!!a.signed_in_at;const sOut=!!a.signed_out_at;const hrs=sIn&&sOut?((new Date(a.signed_out_at)-new Date(a.signed_in_at))/3600000).toFixed(1):null;return(<div key={a.id} style={{ background:'#fff', borderRadius:14, border:'1.5px solid var(--border)', padding:'14px 16px', marginBottom:10 }}><div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:8 }}><div><div style={{ fontSize:15, fontWeight:800, fontFamily:'var(--font-display)', color:'var(--black)', marginBottom:2 }}>{a.session?.title||'Session'}</div><div style={{ fontSize:12, fontWeight:600, color:'var(--muted)', display:'flex', gap:8 }}>{a.session?.date&&<span><i className="ti ti-calendar" style={{ marginRight:3 }} />{fmtDate(a.session.date)}</span>}{a.session?.start_time&&<span><i className="ti ti-clock" style={{ marginRight:3 }} />{a.session.start_time}</span>}</div></div>{hrs&&<div style={{ background:'#E8F7F9', borderRadius:10, padding:'6px 12px', textAlign:'center' }}><div style={{ fontSize:16, fontWeight:900, color:'#1B9AAA', fontFamily:'var(--font-display)' }}>{hrs}h</div></div>}</div><div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>{sIn&&<span style={{ fontSize:11, fontWeight:700, color:'#417505', background:'#EDFAED', borderRadius:20, padding:'3px 10px' }}><i className="ti ti-login" style={{ marginRight:4 }} />In {fmtTime(a.signed_in_at)}</span>}{sOut&&<span style={{ fontSize:11, fontWeight:700, color:'#1B9AAA', background:'#E8F7F9', borderRadius:20, padding:'3px 10px' }}><i className="ti ti-logout" style={{ marginRight:4 }} />Out {fmtTime(a.signed_out_at)}</span>}{!sIn&&a.session?.date===today&&<button onClick={() => handleSignIn(a)} disabled={signingIn===a.id} style={{ padding:'6px 14px', borderRadius:20, border:'none', background:'#417505', color:'#fff', fontSize:11, fontWeight:800, cursor:'pointer' }}>{signingIn===a.id?'...':'Sign In'}</button>}{sIn&&!sOut&&<button onClick={() => handleSignOut(a)} disabled={signingIn===a.id} style={{ padding:'6px 14px', borderRadius:20, border:'none', background:'#1B9AAA', color:'#fff', fontSize:11, fontWeight:800, cursor:'pointer' }}>{signingIn===a.id?'...':'Sign Out'}</button>}</div></div>)})}
    </div>
  )
}
...
export default function VolunteerPortal() {
  ...
  const iStyle = { width:'100%', padding:'11px 14px', borderRadius:10, border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.05)', color:'#fff', fontSize:14, outline:'none', boxSizing:'border-box', marginTop:6 }
  const lStyle = { fontSize:12, fontWeight:600, color:'rgba(255,255,255,0.5)', display:'block' }
    ...
    <div style={{ minHeight:'100vh', background:'#080D1A', display:'flex', alignItems:'center', justifyContent:'center', padding:24, fontFamily:'system-ui,sans-serif' }}>
      <div style={{ width:'100%', maxWidth:400 }}><OrgBadge />
        <form onSubmit={handleRegister} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:20, padding:28 }}>
          ...
          {[['Full name','text','full_name','Jane Smith'],['Email','email','email','jane@email.com'],['Password','password','password','••••••••'],['Confirm password','password','confirm','••••••••']].map(([label,type,key,ph]) => (<div key={key} style={{ marginBottom:14 }}><label style={lStyle}>{label}</label><input type={type} placeholder={ph} value={form[key]} onChange={e => setForm(f=>({...f,[key]:e.target.value}))} required style={iStyle} /></div>))}
    ...
    <div style={{ minHeight:'100vh', background:'#080D1A', display:'flex', alignItems:'center', justifyContent:'center', padding:24, fontFamily:'system-ui,sans-serif' }}>
      <div style={{ width:'100%', maxWidth:400 }}><OrgBadge />
        <form onSubmit={handleLogin} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:20, padding:28 }}>
          ...
          <div style={{ marginBottom:14 }}><label style={lStyle}>Email</label><input type="email" placeholder="your@email.com" value={form.email} onChange={e => setForm(f=>({...f,email:e.target.value}))} required style={iStyle} /></div>
          <div style={{ marginBottom:20 }}><label style={lStyle}>Password</label><input type="password" placeholder="••••••••" value={form.password} onChange={e => setForm(f=>({...f,password:e.target.value}))} required style={iStyle} /></div>
          {error&&<div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:8, padding:'8px 12px', fontSize:13, color:'#FCA5A5', marginBottom:14 }}>{error}</div>}
      ...
      <div style={{ background:'#0D1B2A', padding:'12px 18px 0', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18 }}>
            {!imgFailed?<img src={org?.logo_url||'/logo-horizontal.png'} alt={orgName} style={{ height:26 }} onError={() => setImgFailed(true)} />:<div style={{ fontFamily:'var(--font-display)', fontWeight:900, fontSize:12, color:'#F5D000', letterSpacing:1 }}>{orgName.toUpperCase()}</div>}
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <div onClick={() => setNavTab('profile')} style={{ width:40, height:40, borderRadius:'50%', background:currentPhotoUrl?'transparent':'#9B59B6', border:'2px solid rgba(245,208,0,0.4)', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
                {currentPhotoUrl?<img src={currentPhotoUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />:<span style={{ fontSize:14, fontWeight:900, color:'#fff', fontFamily:'var(--font-display)' }}>{(profile?.full_name||'?').split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2)}</span>}
              </div>
          ...
          <div style={{ marginBottom:6 }}>
            <div style={{ fontSize:24, fontWeight:900, color:'#fff', fontFamily:'var(--font-display)', lineHeight:1.1, marginBottom:6 }}>{greetingWord}, {profile?.full_name?.split(' ')[0]||'there'}! 👋</div>
          ...
          <div style={{ display:'flex', gap:0, margin:'16px -18px 0', borderTop:'1px solid rgba(255,255,255,0.08)' }}>
            {[{val:todaySessions.length||0,label:'Today',color:'#F5D000'},{val:Object.keys(sessionSignups).length,label:'Booked',color:'#fff'},{val:rota.filter(r=>r.session?.date>=today).length,label:'Upcoming',color:'#9FE1CB'}].map((s,i)=>(
              <div key={s.label} style={{ flex:1, padding:'12px 0', textAlign:'center', borderRight:i<2?'1px solid rgba(255,255,255,0.08)':'none' }}>
                <div style={{ fontSize:20, fontWeight:900, color:s.color, fontFamily:'var(--font-display)', lineHeight:1 }}>{s.val}</div>
                <div style={{ fontSize:9, color:'rgba(255,255,255,0.5)', textTransform:'uppercase', letterSpacing:0.5, marginTop:4 }}>{s.label}</div>
              </div>
            ))}
          </div>
      ...
      <div style={{ position:'fixed', bottom:0, left:0, right:0, background:'#fff', borderTop:'1px solid var(--border)', display:'flex', zIndex:100, paddingBottom:'env(safe-area-inset-bottom, 0px)' }}>
        {[{key:'today',icon:'ti-home',label:'Today'},{key:'sessions',icon:'ti-calendar-event',label:'Book'},{key:'rota',icon:'ti-clipboard-list',label:'My Rota'},{key:'register',icon:'ti-heart',label:'Impact'},{key:'profile',icon:'ti-user-circle',label:'Profile'}].map(t=>(
          <button key={t.key} onClick={() => setNavTab(t.key)} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3, padding:'10px 8px', background:'none', border:'none', cursor:'pointer', borderTop:navTab===t.key?`2px solid ${primary}`:'2px solid transparent' }}>
            ...
            <span style={{ fontSize:9, fontWeight:800, color:navTab===t.key?primary:'var(--muted)', fontFamily:'var(--font-display)', textTransform:'uppercase', letterSpacing:0.3 }}>{t.label}</span>
          </button>
        ))}
      </div>
        ...
        {navTab==='today'&&(
          <div>
            <button onClick={() => setShowCFC(true)} style={{ width:'100%', display:'flex', alignItems:'center', gap:10, padding:'12px 14px', background:'#FFF0F0', border:'1.5px solid #FFB3B3', borderRadius:14, cursor:'pointer', marginBottom:16, textAlign:'left' }}>
              ...
              <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:900, color:'#C00', fontFamily:'var(--font-display)' }}>Cause for Concern</div><div style={{ fontSize:11, color:'#C00', fontWeight:600, opacity:0.8 }}>Report a safeguarding concern immediately</div></div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:16 }}>
              {[{val:volunteerHours.toFixed(1),label:'Hours given',color:'#F5D000',bg:'#0D1B2A'},{val:volunteerSessions,label:'Adventures',color:'#9FE1CB',bg:'#0F6E56'},{val:Math.round(volunteerHours*2),label:'Young people',color:'#CECBF6',bg:'#2D1B69'},{val:streak,label:'Week streak',color:'#FAC775',bg:'#633806'}].map(s=>(
                <div key={s.label} style={{ background:s.bg, borderRadius:14, padding:'12px 14px' }}><div style={{ fontSize:24, fontWeight:900, color:s.color, fontFamily:'var(--font-display)', lineHeight:1, marginBottom:3 }}>{s.val}</div><div style={{ fontSize:10, color:'rgba(255,255,255,0.6)', fontWeight:700, textTransform:'uppercase', letterSpacing:0.4 }}>{s.label}</div></div>
              ))}
            </div>
            <div style={{ background:'#fff', borderRadius:18, border:'1.5px solid var(--border)', padding:'14px 16px', marginBottom:16 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}><div style={{ width:28, height:28, borderRadius:8, background:'#FFF8E6', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-trophy" style={{ fontSize:14, color:'#F0A500' }} /></div><span style={{ fontSize:13, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)' }}>Volunteer Journey</span></div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}><div style={{ width:28, height:28, borderRadius:8, background:'#F0A500', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-sun" style={{ fontSize:14, color:'#fff' }} /></div><div style={{ fontSize:13, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)', textTransform:'uppercase', letterSpacing:0.8 }}>Today's Sessions</div></div>
            {todaySessions.length===0?<div style={{ background:'#0D1B2A', borderRadius:18, padding:'28px 20px', textAlign:'center' }}><div style={{ width:60, height:60, borderRadius:18, background:'rgba(245,208,0,0.15)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 14px', border:'1.5px solid rgba(245,208,0,0.2)' }}><i className="ti ti-star" style={{ fontSize:28, color:'#F5D000' }} /></div><div style={{ fontSize:17, fontWeight:900, fontFamily:'var(--font-display)', color:'#fff', marginBottom:6 }}>Rest day!</div><div style={{ fontSize:12, color:'rgba(255,255,255,0.6)', fontWeight:600, lineHeight:1.5 }}>No sessions today — enjoy the break!</div></div>:todaySessions.map(s=>{const cfg=typeConfig[s.session_type]||typeConfig.activity;const count=sessionVolCounts[s.id]||0;const isFull=s.volunteer_limit&&count>=s.volunteer_limit;return(<div key={s.id} style={{ background:'#fff', borderRadius:18, border:'1.5px solid var(--border)', padding:'16px', marginBottom:12, overflow:'hidden', position:'relative' }}><div style={{ position:'absolute', top:0, left:0, width:4, height:'100%', background:cfg.color, borderRadius:'4px 0 0 4px' }} /><div style={{ paddingLeft:8 }}><div style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:10 }}><div style={{ width:46, height:46, borderRadius:14, background:cfg.color+'20', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, border:'1.5px solid '+cfg.color+'40' }}><i className={'ti '+cfg.icon} style={{ fontSize:22, color:cfg.color }} /></div><div style={{ flex:1 }}><div style={{ fontSize:16, fontWeight:900, fontFamily:'var(--font-display)', color:'var(--black)', marginBottom:4 }}>{s.title}</div><div style={{ fontSize:12, fontWeight:600, color:'var(--muted)', display:'flex', gap:10, flexWrap:'wrap' }}><span style={{ display:'flex', alignItems:'center', gap:4 }}><i className="ti ti-clock" style={{ fontSize:12 }} />{s.start_time} – {s.end_time}</span>{s.location&&<span style={{ display:'flex', alignItems:'center', gap:4 }}><i className="ti ti-map-pin" style={{ fontSize:12 }} />{s.location.split(',')[0]}</span>}</div></div></div><div style={{ display:'flex', gap:8, flexWrap:'wrap' }}><span style={{ fontSize:11, fontWeight:800, background:'#F5F0FF', color:'#9B59B6', borderRadius:99, padding:'4px 12px', display:'flex', alignItems:'center', gap:4 }}><i className="ti ti-heart-handshake" style={{ fontSize:11 }} />{count} volunteer{count!==1?'s':''}</span>{s.volunteer_limit&&<span style={{ fontSize:11, fontWeight:700, background:isFull?'#FFF0F0':'#EDFAED', color:isFull?'#C00':'#417505', borderRadius:99, padding:'4px 12px' }}>{isFull?'Full':`${s.volunteer_limit-count} spots left`}</span>}</div></div></div>)})}
          </div>
        )}
      ...
      {showCFC&&(
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'flex-end' }}>
          <div style={{ background:'#fff', borderRadius:'20px 20px 0 0', width:'100%', maxHeight:'90vh', overflowY:'auto' }}>
            <div style={{ padding:'16px 20px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}><div style={{ fontSize:16, fontWeight:900, color:'var(--black)', fontFamily:'var(--font-display)' }}>Cause for Concern</div><button onClick={() => setShowCFC(false)} style={{ background:'var(--grey-bg)', border:'none', borderRadius:'50%', width:32, height:32, cursor:'pointer', fontSize:18, color:'var(--muted)' }}>×</button></div>
            <div style={{ padding:20 }}>
              <p style={{ fontSize:14, color:'var(--muted)', marginBottom:16 }}>Use this form to report any safeguarding concerns. Your submission will be sent to the DSL immediately.</p>
              <textarea rows={6} placeholder="Describe the concern in detail..." style={{ width:'100%', padding:'12px 14px', borderRadius:12, border:'1.5px solid var(--border)', fontSize:14, outline:'none', boxSizing:'border-box', resize:'none', marginBottom:14 }} />
              <button onClick={() => setShowCFC(false)} style={{ width:'100%', padding:14, borderRadius:14, border:'none', background:'#C00', color:'#fff', fontSize:15, fontWeight:900, cursor:'pointer' }}>Submit Concern</button>
            </div>
          </div>
        </div>
      )}
