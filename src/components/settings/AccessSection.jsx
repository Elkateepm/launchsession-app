  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 620 }}>
            <thead>
              <tr>
                <th style={{ textAlign: 'left', padding: '12px 16px', fontSize: 11, fontWeight: 800, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: 1, borderBottom: '1px solid var(--border)' }}>Module</th>
                {TEMPLATABLE_ROLES.map(r => (
                  <th key={r.key} style={{ textAlign: 'left', padding: '12px 10px', fontSize: 11, fontWeight: 800, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: 1, borderBottom: '1px solid var(--border)' }}>{r.label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {ACCESS_MODULES.map(m => (
                <tr key={m.key}>
                  <td style={{ padding: '10px 16px', borderBottom: '1px solid var(--border-soft)', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-faint)', marginTop: 14, lineHeight: 1.7 }}>
        <strong style={{ color: 'var(--text3)' }}>Default</strong> leaves the role as it has always behaved — it does not
