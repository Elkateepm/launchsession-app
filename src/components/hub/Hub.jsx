...
function HistoricalAttendanceModal({ session, attendance, allChildren, primary, secondary, onClose }) {
  const isMobile = useIsMobile()
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {rows.map(r => {
                ...
                if (r.att.status === 'signed_out' && r.att.signed_in_at) {
                  timeLine = `In ${hubFmtTime(r.att.signed_in_at)} → Out ${hubFmtTime(r.att.signed_out_at)}`
                } else if (r.att.status === 'signed_in' && r.att.signed_in_at) {
                  timeLine = `In ${hubFmtTime(r.att.signed_in_at)}`
