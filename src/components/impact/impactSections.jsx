...
export const card = {
  background: 'var(--surface)', border: '1px solid #EEF0F2', borderRadius: 20,
  padding: '20px 22px', boxSizing: 'border-box', minWidth: 0,
}

export const sectionTitle = { fontSize: 14, fontWeight: 900, color: 'var(--text)' }
export const sectionHint = { fontSize: 12, color: 'var(--text-faint)', marginTop: 2, lineHeight: 1.45 }
    ...
    <div style={{ ...card, padding: isMobile ? '22px 18px' : '26px 24px' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: isMobile ? 26 : 32, fontWeight: 900, color: 'var(--text)', letterSpacing: -0.6, lineHeight: 1.15 }}>
        ...
        {avgDelta !== null && (
          <div style={{ textAlign: isMobile ? 'left' : 'right', flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--text-faint)', fontWeight: 700 }}>average change, out of 10</div>
          </div>
        )}
      </div>

      <div style={{ display: 'flex', height: 12, borderRadius: 99, overflow: 'hidden', marginTop: 18, background: 'var(--surface-hover)' }}>
        {segments.map(s => (
      ...
      <div style={{ ...sectionHint, marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-soft)' }}>
...
export function AreaMovement({ impact, isMobile, onSelectArea, activeArea }) {
  ...
  return (
    <div style={card}>
          ...
          return (
            <motion.button
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
                <span style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {meta.label}
                </span>
                <span style={{ fontSize: 10.5, color: 'var(--text-faint)', fontWeight: 700, flexShrink: 0 }}>{a.people}</span>
              </span>

              <span style={{ position: 'relative', height: 22, display: 'block' }}>
                <span style={{ position: 'absolute', top: 10, left: 0, right: 0, height: 2, background: 'var(--surface-hover)', borderRadius: 99 }} />
                <motion.span
                ...
                <span style={{ position: 'absolute', top: 5, left: `calc(${a.baseline / 10 * 100}% - 6px)`, width: 12, height: 12, borderRadius: 99, background: 'var(--surface)', border: `2px solid ${GREY}`, boxSizing: 'border-box' }} />
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-soft)', flexWrap: 'wrap' }}>
    ...
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: 22, marginTop: 16, flexWrap: 'wrap' }}>
        {stats.map(s => (
          <div key={s.label}>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--text-faint)', fontWeight: 700 }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      {goals.recent.length > 0 && (
        <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid var(--border-soft)' }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-faint)', letterSpacing: 0.3, marginBottom: 8 }}>
...
export function NeedsAttention({ impact, childName, nameFor, onOpenChild, terms }) {
  ...
  const Row = ({ id, primaryText, secondaryText, tone }) => (
    <button onClick={() => onOpenChild(id)} style={{
      ...
      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{primaryText}</span>
      <span style={{ marginLeft: 'auto', fontSize: 11.5, color: 'var(--text-faint)', fontWeight: 700, flexShrink: 0 }}>{secondaryText}</span>
    </button>
  )
    ...
    <div style={card}>
      ...
      {declined.length > 0 && (
        <div style={{ marginTop: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-faint)', letterSpacing: 0.3, marginBottom: 4 }}>SCORES FELL</div>
      ...
      {quiet.length > 0 && (
        <div style={{ marginTop: declined.length > 0 ? 14 : 10 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-faint)', letterSpacing: 0.3, marginBottom: 4 }}>
        ...
        {usable.map(r => {
          ...
          return (
            <div key={r.name} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '150px 1fr 52px', alignItems: 'center', gap: isMobile ? 4 : 14 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.name}</div>
                <div style={{ fontSize: 10.5, color: 'var(--text-faint)', fontWeight: 700 }}>{r.improved} of {r.people} improved</div>
              </div>
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 3, background: 'var(--surface2)', borderRadius: 10, padding: 3, flexWrap: 'wrap' }}>
          {SORTS.map(s => (
      ...
      <div style={{ marginTop: 12 }}>
        {rows.length === 0 && (
          <div style={{ padding: '26px 0', textAlign: 'center', color: 'var(--text-faint)', fontSize: 13, fontWeight: 600 }}>
            Nobody matches.
          </div>
        )}
        {rows.map(({ child, m }) => (
          <button key={child.id} onClick={() => onOpenChild(child.id)} style={{
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0 }}>
              ...
              <span style={{ minWidth: 0 }}>
                ...
                <span style={{ display: 'block', fontSize: 11, color: 'var(--text-faint)', fontWeight: 600 }}>
            ...
            {!isMobile && (
              <span style={{ fontSize: 12, color: 'var(--text-faint)', fontWeight: 700 }}>
                {m ? <>{m.baseline.toFixed(1)} <Icon name="→" /> <span style={{ color: 'var(--text2)', fontWeight: 800 }}>{m.latest.toFixed(1)}</span></> : '—'}
              </span>
            )}
