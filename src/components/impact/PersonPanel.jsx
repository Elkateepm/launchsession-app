...
export default function PersonPanel({ child, org, scores, onClose, onRecordOutcome, isAdmin }) {
  ...
  const inp = { width: '100%', boxSizing: 'border-box', padding: '9px 12px', borderRadius: 10, border: '1.5px solid var(--border)', fontSize: 13, fontFamily: 'inherit', outline: 'none' }
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(480px, 100vw)', background: 'var(--surface)', zIndex: 901, boxShadow: '-20px 0 60px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column' }}
      >
        {/* Header */}
        <div style={{ background: `linear-gradient(135deg, var(--org-a10), var(--org-a05))`, padding: '22px 22px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <button onClick={onClose} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, width: 32, height: 32, cursor: 'pointer', fontSize: 14, color: 'var(--text3)' }}><Icon name="✕" /></button>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 22 }}>
          ...
          {tab === 'timeline' && (
            childScores.length === 0 ? (
              ...
              <div style={{ position: 'relative', paddingLeft: 20 }}>
                <div style={{ position: 'absolute', left: 5, top: 6, bottom: 6, width: 2, background: 'var(--surface3)' }} />
                {childScores.map((s, i) => {
                  ...
                  return (
                    <div key={s.id} style={{ position: 'relative', marginBottom: i === childScores.length - 1 ? 0 : 18 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-faint)', fontWeight: 700 }}>{format(new Date(s.recorded_at), 'd MMM yyyy')} · {formatDistanceToNow(new Date(s.recorded_at), { addSuffix: true })}</div>
                      ...
                      {s.notes && <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 4, lineHeight: 1.5 }}>{s.notes}</div>}
                    </div>
                  )
                })}
              </div>
          ...
          {tab === 'goals' && (
            <div>
              {isAdmin && (
                <div style={{ background: 'var(--surface2)', borderRadius: 14, padding: 14, marginBottom: 16, border: '1px solid var(--border-soft)' }}>
              ...
              {goalsLoading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {goals.map(g => {
                    ...
                    return (
                      <div key={g.id} style={{ border: '1px solid var(--border-soft)', borderRadius: 14, padding: 14, opacity: g.status === 'completed' ? 0.7 : 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <div>
          ...
          {tab === 'sessions' && (
            attLoading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {attendance.map(a => (
                  <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', border: '1px solid var(--border-soft)', borderRadius: 12 }}>
                    <div>
            ...
            childScores.filter(s => s.notes).length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {childScores.filter(s => s.notes).map(s => {
                  const a = areaByKey(s.area)
                  return (
                    <div key={s.id} style={{ border: '1px solid var(--border-soft)', borderRadius: 14, padding: 14 }}>
                      <div style={{ fontSize: 11, color: 'var(--text-faint)', fontWeight: 700 }}>{a.icon} {a.label} · {format(new Date(s.recorded_at), 'd MMM yyyy')}</div>
            ...
            (() => {
              ...
              return (
                <div>
                  ...
                  {locked.length > 0 && (
                    <>
                      <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-faint)', marginBottom: 10 }}>LOCKED</div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                        {locked.map(a => (
                          <div key={a.key} style={{ background: 'var(--surface2)', border: '1.5px dashed var(--border)', borderRadius: 14, padding: '14px 12px', textAlign: 'center', opacity: 0.6 }}>
