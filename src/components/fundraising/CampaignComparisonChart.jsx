...
export default function CampaignComparisonChart({ campaigns }) {
  ...
  if (withTargets.length === 0) {
    return (
      <div style={{ border: '1px dashed #E5E3DC', borderRadius: 16, padding: '18px 20px', marginBottom: 20, background: 'var(--surface)' }}>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--text-faint)', lineHeight: 1.5 }}>Set a target amount on a campaign to track progress here.</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ border: '1px solid #E5E3DC', borderRadius: 16, padding: '18px 20px', marginBottom: 20, background: 'var(--surface)' }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 16 }}>{withTargets.length === 1 ? 'Add a second campaign with a target to compare them side by side' : 'Progress toward target, side by side'}</div>
