...
export default async function handler(req, res) {
  try {
    ...
    // Retired endpoints must fail even for cached clients from older releases.
    if (typeof req.body?.type === 'string' && req.body.type.startsWith('passkey_')) {
      return res.status(410).json({ error: 'Passkey sign-in has been removed. Please sign in with your email and password.' })
    }
